...
public class ColumnCardsDeleteFailedConsumer: BackgroundService
{
    ...
    public ColumnCardsDeleteFailedConsumer(IServiceScopeFactory scopeFactory, IOptions<KafkaSettings> options)
    {
        ...
        _topic = _kafkaSettings.DeleteColumnFailedTopic;
    }
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            var evt = JsonSerializer.Deserialize<ColumnDeleteEvent>(cr.Message.Value);

            if (evt != null)
            {
                using var scope = _scopeFactory.CreateScope();
                var boardService = scope.ServiceProvider.GetRequiredService<IColumnService>();

                await boardService.FailedColumnDeleteAsync(evt.ColumnId);
            }
        }
    }
